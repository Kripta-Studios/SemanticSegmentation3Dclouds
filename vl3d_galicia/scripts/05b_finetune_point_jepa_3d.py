...
from scripts.train_common import balanced_block_sampler, class_weights_from_blocks, infer_channel_layout, load_config, model_parameter_summary, parse_class_boost, save_json, segmentation_optimizer_groups, set_global_seed
from src.data.segmentation_dataset import SegmentationBlockDataset, segmentation_collate_fn
from src.models.segmentation.heads import PointJepa3DSegmentationNet, GatedExternalPointJepa3DSegmentationNet
from src.training.segmentation_trainer import SegmentationTrainer

def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune segmentation from a pretrained PointJEPA3D encoder.")
    ...
    out = Path(args.out or cfg.get("output", {}).get("dir", "outputs/point_jepa_3d_finetune"))
    ...
    if complete_path.exists() and best_model_path.exists() and not args.no_resume:
        ...
        if int(complete.get("epochs_completed", 0)) >= epochs or bool(complete.get("early_stopped", False)):
            print(f"PointJEPA3D fine-tuning already complete at {complete_path}; skipping training.")
            return
    ...
    if args.external_feature_dir:
        # Assuming DINO features are 384 dimensions for ViT-S
        external_in_channels = 384
        model = GatedExternalPointJepa3DSegmentationNet(
            base_in_channels=in_channels,
            external_in_channels=external_in_channels,
            embed_dim=256
        )
    else:
        model = PointJepa3DSegmentationNet(in_channels=in_channels, embed_dim=256)
    ...
    train_ds = SegmentationBlockDataset(train_dir, max_blocks=max_train, **ds_kwargs)
    val_ds = SegmentationBlockDataset(val_dir, max_blocks=max_val, **ds_kwargs)
    test_ds = SegmentationBlockDataset(test_dir, **ds_kwargs) if Path(test_dir).exists() else None
    ...
    run_config = {
        "model_name": out.name,
        "experiment_type": "point_jepa_3d_downstream",
    ...
    if checkpoint_path.exists() and not args.no_resume:
        ...
        print(f"Resuming PointJEPA3D fine-tuning from epoch {start_epoch}/{epochs}")
        
    trainer.fit(
        start_epoch=start_epoch,
        checkpoint_path=checkpoint_path,
        best_model_path=best_model_path,
        progress_label="PointJEPA3D fine-tuning",
    )
